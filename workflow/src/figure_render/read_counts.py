"""Read count distribution figure rendering.

Input
-----
- Binned distribution TSV with columns ``sample``, ``timepoint``,
  ``bin_left``, ``bin_right``, ``count``.
- Cutoff retention statistics TSV with columns ``sample``, ``original_rows``,
  ``rows_kept``, ``pct_rows_kept``, ``original_counts``, ``counts_kept``,
  ``pct_counts_kept``.

Output
------
- ``<stem>.pdf`` / ``<stem>.review.png`` — pre-binned log10 read-count
  histograms, one panel per sample-timepoint group, with the hard-filtering
  cutoff marked on the initial-timepoint panel.
